...
docker run -p 8080:8080 -v ~/.local/share/next-plaid:/data/indices \
  ghcr.io/lightonai/next-plaid:cpu-1.2.0 \
...
docker run --gpus all -p 8080:8080 -v ~/.local/share/next-plaid:/data/indices \
  ghcr.io/lightonai/next-plaid:cuda-1.2.0 \
  --host 0.0.0.0 --port 8080 --index-dir /data/indices \
...
```bash
# CPU (amd64 + arm64)
docker pull ghcr.io/lightonai/next-plaid:cpu-1.2.0

# CUDA (amd64, requires NVIDIA GPU)
docker pull ghcr.io/lightonai/next-plaid:cuda-1.2.0
```
...
services:
  next-plaid-api:
    image: ghcr.io/lightonai/next-plaid:cpu-1.2.0
    ports:
      - "8080:8080"
    volumes:
...
services:
  next-plaid-api:
    image: ghcr.io/lightonai/next-plaid:cuda-1.2.0
    ports:
      - "8080:8080"
    volumes:
      - ${NEXT_PLAID_DATA:-~/.local/share/next-plaid}:/data/indices
